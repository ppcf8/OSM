...
% Fitness Evaluation
pop_final=0;
[fitness,X_pop,Population] = fitness_function(Population,N_pop,lb,ub,l_c,f);
...
while k<kmax && std(fitness)>tol
    ...
    [fitness_offsprings,~,Offsprings_population] = fitness_function(Offsprings_population,size(Offsprings_population,1),lb,ub,l_c,f);

    [fitness,~,Population] = fitness_function(Population,N_pop,lb,ub,l_c,f);
